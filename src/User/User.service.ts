...
import { UserProfile } from '../Profile/profile/entites/user-profile.entity';
  ...
  export class UsersService {
    constructor(
      ...
      @InjectRepository(UserProfile) private readonly userprofileRespository:Repository<UserProfile>,
...
async VerifiedUser(data:any){
 ...
 async updateRefreshToken(refreshToken, expirydate, payload) {
  ...
  await this.refreshTokenRepository.upsert(dataToUpsert, uniqueIdentifierFields);
 ...
 async refresh(data) {
  ...
  if (!user) {
    ...
    if (expirydate < new Date()) {
      throw new UnauthorizedException({ code: 4, error: 'Refresh Token Expired' });
    }else{
      const payload = {
        userId: user.userId
      };
...
async passwordupdate(rsdata: any) {
  ...
  if (userdetails) {
    ...
    if (matchpassword) {
      ...
      const updatepassword=this.userRepository.update({id:rsdata.userId},{password:newhashpassword})
      if (updatepassword) {
...
async forgotTokenverifiedbyemil(data: any) {
  ...
  if(!!verifiedtoken){
    const tokenExpirationTime = new Date(verifiedtoken.created_at.getTime() + 60 * 60 * 1000);/// expire token in 1hr
    if (currentTime >=tokenExpirationTime) {
      const resethashpassword = await bcrypt.hash(data.resetpassword, 10);
      const res = await this.userRepository.update(verifiedtoken.id, { password: resethashpassword });
      if (!!res) {
        return { success: true, message: 'Password updated successfully' };
      }else{
        throw new InternalServerErrorException("something went wrong");
