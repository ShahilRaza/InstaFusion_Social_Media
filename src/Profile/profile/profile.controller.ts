import {BadRequestException, Body, Controller, Delete, Get, Headers,NotFoundException, Param, Post, Put, Query, Request, UploadedFile, UploadedFiles, UseGuards, UseInterceptors  } from '@nestjs/common';
...
export class ProfileController {
    ...
    @UseInterceptors(FileFieldsInterceptor([
     ...
     async createuserprofile(@UploadedFiles() files: { profile},@Body() data: userprofileDto,@Request() req){
      if (!!files) {
        if (!!files.profile) {
            if (!files.profile[0].originalname.match(/\.(jpg|JPG|jpeg|JPEG|png|PNG)$/)) {
    ...
    async UpdateUserprofile(@UploadedFiles() files: { profile},@Param('id') id: string,@Body() data: UserprofiledDto) {
      if (!!files) {
        if (!!files.profile) {
            if (!files.profile[0].originalname.match(/\.(jpg|JPG|jpeg|JPEG|png|PNG)$/)) {
                throw new BadRequestException('Invalid Image type');
      ...
      return await this.UserprofileService.UpdateUserProfileService({id,data,files})
